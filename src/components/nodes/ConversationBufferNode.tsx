...
export function ConversationBufferNode({ id, data, selected }: NodeProps) {
  ...
  return (
    <div className={`node-shell ${statusClass}`} style={{ width: (data as unknown as Record<string, unknown>).width as number ?? 224 }}>
      ...
      <div className="node-header bg-indigo-700 flex justify-between items-center" onDoubleClick={() => updateNodeData(id, { collapsed: !collapsed })}>
        <span>{collapsed ? '▸' : '▾'} 💬 Chat Buffer</span>
