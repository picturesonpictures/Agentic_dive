...
export function ImageInputNode({ id, data, selected }: { id: string; data: Record<string, unknown>; selected?: boolean }) {
  ...
  return (
    <div className="node-shell" style={{ width: (data as Record<string, unknown>).width as number ?? 224 }}>
      ...
      <div className="node-header bg-purple-700"
        onDoubleClick={() => update(id, { collapsed: !collapsed })}>
        {collapsed ? '▸' : '▾'} 🖼️ {d.label || 'Image Input'}
      </div>
