...
export function JsonExtractNode({ id, data, selected }: NodeProps) {
  const d = data as unknown as JsonExtractData
  const updateNodeData = useFlowStore(s => s.updateNodeData)
  const collapsed = !!(data as unknown as Record<string, unknown>).collapsed

  return (
    <div className="node-shell" style={{ width: (data as Record<string, unknown>).width as number ?? 208 }}>
      ...
      <div className="node-header bg-rose-700"
        ...
        <span>{collapsed ? '▸' : '▾'} 🔍 JSON Extract</span>
      </div>
